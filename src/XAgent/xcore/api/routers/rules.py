...
async def check_channel_exists(channel_id: str, engine: RuleEngineOrchestrator) -> bool:
    """检查通道是否存在（数据库或router）
    ...
    """
    # 先从数据库检查
    if engine._persistence_manager:
        try:
            channels = await engine._persistence_manager.load_all_channels()
            if channel_id in channels:
                return True
        except Exception as e:
            logger.error(f"Failed to check channel in persistence: {e}")
    
    # 再从router检查（兼容旧版本）
    return engine.is_channel_registered(channel_id)


def validate_channel_config(plugin_name: str, config: dict) -> None:
    """验证通道配置格式（仅基本验证）
    ...
    if plugin_name == 'email':
        ...
        if smtp_port is not None:
            if not isinstance(smtp_port, int) or smtp_port < 1 or smtp_port > 65535:
                raise HTTPException(
                    status_code=400,
                    detail="smtp_port must be an integer between 1 and 65535"
                )
        ...
        if recipients is not None and not isinstance(recipients, list):
            raise HTTPException(
                status_code=400,
                detail="recipients must be a list"
            )


async def get_channel_config_from_db(
    channel_id: str, 
    ...
    Returns:
        Optional[Dict]: 通道配置，不存在返回None
    """
    if not engine._persistence_manager:
        return None
    
    try:
        channels = await engine._persistence_manager.load_all_channels()
        if channel_id in channels:
            record = channels[channel_id]
            return {
                'plugin_name': record.plugin_name,
                'config': record.config
            }
    except Exception as e:
        logger.error(f"Failed to load channel from persistence: {e}")
...
async def list_channels(
    ...
    ✅ 优化：优先从数据库加载，fallback到router（兼容旧版本）
    """
    channels = []
    ...
    if engine._persistence_manager:
        try:
            all_channels = await engine._persistence_manager.load_all_channels()
            for channel_id, record in all_channels.items():
            ...
            logger.error(f"Failed to load channels from persistence: {e}")
    
    # 如果数据库为空，从router获取（兼容旧版本）
    if not channels:
...
@router.put("/channels/{channel_id}", response_model=ChannelOperationResponse)
async def update_channel(
    ...
    """
    # 1. 检查通道是否存在
    if not await check_channel_exists(channel_id, engine):
        raise HTTPException(status_code=404, detail=f"Channel '{channel_id}' not found")

    # 2. 验证插件是否存在
    plugin_key = f"rule_engine.delivery:{request.plugin_name}"
    if not engine.plugin_manager.get_plugin_class(plugin_key):
        raise HTTPException(
            status_code=400,
    ...
    # 3. 验证配置格式（仅基本验证）
    validate_channel_config(request.plugin_name, request.config)

    # 4. 更新通道配置
    success = await engine.update_delivery_channel_async(
        channel_id=channel_id,
        plugin_name=request.plugin_name,
        config=request.config
    )
    ...
    if not success:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update channel '{channel_id}'"
        )
...
async def test_channel(
    channel_id: str,
    request: Optional[ChannelTestRequest] = None,
    engine: RuleEngineOrchestrator = Depends(get_rule_engine),
    token: str = Depends(verify_api_token)
):
    """测试通知通道连接
    ...
    try:
        # 模式1：测试已注册的通道
        if engine.is_channel_registered(channel_id):
            plugin = engine.router.get_channel_plugin(channel_id)
            if not plugin:
                raise HTTPException(
                    status_code=404,
                    detail=f"Plugin for channel '{channel_id}' not found"
                )

            success = await plugin.test_connection()
            return ChannelTestResponse(
                success=success,
                message="Connection test passed" if success else "Connection test failed",
                channel_id=channel_id,
            )
        
        # 模式2：测试数据库中的通道（未启用）
        channel_data = await get_channel_config_from_db(channel_id, engine)
        if not channel_data:
            raise HTTPException(
                status_code=404,
        ...
        # 获取插件名称
        plugin_name = request.plugin_name or channel_data['plugin_name']
        if not plugin_name:
            raise HTTPException(
                status_code=400,
                detail="plugin_name is required"
            )

        # 创建临时插件实例并测试
        plugin_key = f"rule_engine.delivery:{plugin_name}"
        plugin_class = engine.plugin_manager.get_plugin_class(plugin_key)
        if not plugin_class:
            raise HTTPException(
                status_code=400,
        ...
        temp_plugin.initialize(channel_data['config'])
        success = await temp_plugin.test_connection()
