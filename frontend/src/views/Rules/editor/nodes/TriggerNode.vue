...
.rule-node {
  ...
  border: 1.5px solid var(--node-trigger-border);
  border-radius: 14px;
  padding: 0;
  box-shadow:
...
.rule-node:hover {
  ...
  box-shadow:
    ...
    0 0 20px var(--node-trigger-glow),
...
:deep(.vue-flow__node.selected) .rule-node {
  box-shadow:
    0 0 0 2px var(--node-trigger-border),
    0 0 30px var(--node-trigger-glow),
...
.node-glow {
  ...
  inset: -3px;
  border-radius: 16px;
  background: radial-gradient(ellipse at center, var(--node-trigger-glow) 0%, transparent 70%);
  z-index: -1;
  opacity: 0.6;
  pointer-events: none;
  filter: blur(12px);
...
.rule-node :deep(.vue-flow__handle) {
  width: 12px;
  height: 12px;
  background: var(--re-handle-bg);
  border: 2px solid var(--node-trigger-border);
  box-shadow: 0 0 8px var(--node-trigger-glow);
  transition: box-shadow 0.2s, background 0.2s;
}

.rule-node :deep(.vue-flow__handle:hover) {
  box-shadow: 0 0 14px var(--node-trigger-glow), 0 0 4px var(--node-trigger-glow);
  background: var(--node-trigger-border);
}
</style>
