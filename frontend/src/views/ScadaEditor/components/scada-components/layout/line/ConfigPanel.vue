<template>
  <div class="config-section">
    <div class="section-title">{{ t('componentConfig.lineConfig') }}</div>
    ...
    <div class="form-row">
      <div class="form-group">
        <label>{{ t('componentConfig.lineAngle') }}</label>
        <input
          type="number"
          :value="config.angle"
          @change="updateConfig('angle', +($event.target as HTMLInputElement).value)"
        />
      </div>
      <div class="form-group">
...
import type { ScadaComponent, LineComponentConfig } from '../../../../types'
...
const { config, updateConfig } = useScadaConfig(
  props.component as ScadaComponent<'line'>,
)
...
const handleArrowChange = (val: string) => {
  updateConfig('arrow', val as LineComponentConfig['arrow'])
}
</script>
