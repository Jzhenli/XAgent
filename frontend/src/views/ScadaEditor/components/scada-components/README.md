...
├── component-registry.ts # 统一导出（类型 + 注册表 API）
├── components/
│   └── scada-components/
│       ├── basic/        # 基础组件分类
│       │   ├── button/   # 按钮组件
...
**registry.ts**（位于 `ScadaEditor/registry.ts`）— 添加配置接口、导入元数据、注册映射：
...
// 2. 导入元数据（注意分类目录路径）
import { myComponentMeta } from './components/scada-components/control/my-component/metadata'
...
**metadata.ts** — 从 `ScadaEditor/types.ts` 导入公共类型（注意相对路径层级）：

```typescript
import type { ScadaComponentMeta, MyComponentConfig } from '../../../../types'
...
> - 配置接口统一在 `types.ts` 中定义，无需修改 `component-registry.ts`
> - 所有组件都放在分类目录下的子目录中，metadata.ts 中相对路径为 `../../../../types`
...
| 文件 | 职责 |
|------|------|
| `ScadaEditor/types.ts` | 公共类型（StyleConfig、ScadaComponentMeta、各组件配置接口） |
| `ScadaEditor/registry.ts` | 统一注册表 + 查询/注册 API |
| `ScadaEditor/component-registry.ts` | 统一导出（类型 + 注册表 API） |
