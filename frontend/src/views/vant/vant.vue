<template>
  <div class="vant-container">
    ...
    <div v-else-if="panels.length > 0" class="slide-wrapper">
      <div
...
// ============== 滑动翻页（无动画） ==============
/** 本次触摸判定的滑动方向：1 = 左滑（下一页），-1 = 右滑（上一页），null = 未触发 */
let swipeDir: -1 | 1 | null = null;
/** 手势起点处面板横向滚动位置：结束时对比，识别"手势被内容横向滚动消费" */
...
const handleTouchStart = (e: TouchEvent) => {
  swipeStartScrollLeft = adaptContainerRef.value?.scrollLeft ?? 0;
  swipeDir = null;
  onTouchStart(e);
};
...
/**
 * 手势结束：滑动未被面板内容横向滚动消费时才翻页
 * （fitHeight 面板内容可横向滚动，滚动手势不应触发切换；
 *  滚动到边界后继续同向滑动则正常翻页）
 */
const handleTouchEnd = (e: TouchEvent) => {
  ...
  if (dir === null) return;
  const scrolled =
    Math.abs((adaptContainerRef.value?.scrollLeft ?? 0) - swipeStartScrollLeft) > 1;
  if (!scrolled) switchTab(dir);
};

/** 手势被系统接管（来电、通知中心等）：仅重置手势状态 */
