  ...
  >
    ...
    <div v-show="currentStep === 2" class="step-content">
      ...
      <el-card shadow="never">
        ...
        <el-empty
          ...
          :image-size="60"
        >
          <el-button type="primary" size="small" @click="handleResearch">{{
    ...
    <template #footer>
      ...
      <template v-else-if="currentStep === 1">
        <el-button size="small" @click="handleClose" :disabled="searching">{{
          t("common.cancel")
        }}</el-button>
        <el-button size="small" type="primary" :loading="true"
...
const handleDiscoverDevices = async () => {
  ...
  try {
    ...
    if (deviceIdRangeMin.value !== null && deviceIdRangeMax.value !== null) {
      request.device_id_range = [
        deviceIdRangeMin.value,
        deviceIdRangeMax.value,
      ];
    }
...
const handleBatchAdd = async () => {
  ...
  try {
    ...
    const devicesToAdd: DeviceConfig[] = selectedDevices.value.map(
      (device) => ({
        asset: `bacnet_${device.device_id}`,
        name: device.device_name || `BACnet Device ${device.device_id}`,
        description: `${device.vendor_name || "Unknown"} - ${device.model_name || "Unknown"}`,
    ...
    if (response.succeeded > 0) {
      ElMessage.success(
        t("devices.batchAddSuccess", { count: response.succeeded }),
      );
      emit("success");
      handleClose();
    } else {
    ...
    if (response.failed > 0) {
      ElMessage.warning(
        t("devices.batchAddPartialFailed", { count: response.failed }),
      );
    }
  } catch (error: any) {
    if (error !== "cancel") {
    ...
    if (visible) {
      try {
        ...
        if (networkInterfaces.value.length > 0) {
          ...
          console.log(
            "自动选择最高优先级网卡:",
            networkInterfaces.value[0].name,
            networkInterfaces.value[0].ip_address,
          );
        } else {
